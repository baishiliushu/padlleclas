...
def save_model(net,
    ...
    if prefix == 'best_model':
        ...
        # 静态图保存
        # x_spec = InputSpec(shape=[None,3,256,192], name='x')  # 动态 shape
        # y_spec = InputSpec(shape=[None,1,26], name='y')
        #
        # net = paddle.jit.to_static(net, input_spec=[x_spec])
        # # net = paddle.jit.to_static(input_spec=[InputSpec(shape=[None, 10], name='x')])
